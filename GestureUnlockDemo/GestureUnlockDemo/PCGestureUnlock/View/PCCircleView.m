 ...
 *  连接的圆最少的个数
 */
static NSInteger CircleSetCountLeast = 4;
...
- (UIColor *)CircleConnectLineNormalColor {
    if (_CircleConnectLineNormalColor == nil) {
        _CircleConnectLineNormalColor = [UIColor colorWithRed:34/255.0 green:178/255.0 blue:246/255.0 alpha:1];
    }
    return _CircleConnectLineNormalColor;
}

- (UIColor *)CircleConnectLineErrorColor {
    if (_CircleConnectLineErrorColor) {
        _CircleConnectLineErrorColor = [UIColor colorWithRed:254/255.0 green:82/255.0 blue:92/255.0 alpha:1];
    }
    return _CircleConnectLineErrorColor;
}

- (float)CircleConnectLineWidth {
    if (_CircleConnectLineWidth == 0) {
        _CircleConnectLineWidth = 1.0f;
    }
    return _CircleConnectLineWidth;
}
...
- (void)gestureEndByTypeSettingWithGesture:(NSString *)gesture length:(CGFloat)length
{
    if (length < CircleSetCountLeast) {
        ...
        if ([gestureOne length] < CircleSetCountLeast) { // 接收并存储第一个密码
            // 记录第一次密码
            [PCCircleViewConfig saveGesture:gesture Key:gestureOneSaveKey];
            ...
            BOOL equal = [gesture isEqual:[PCCircleViewConfig getGestureWithKey:gestureOneSaveKey]]; // 匹配两次手势
            // 通知代理
...
{
    NSString *password = [PCCircleViewConfig getGestureWithKey:gestureFinalSaveKey];
...
- (void)drawRect:(CGRect)rect
{
    ...
    if ([self getCircleState] == CircleStateError) {
        color = self.CircleConnectLineErrorColor;
    } else {
        color = self.CircleConnectLineNormalColor;
    }
...
- (void)connectCirclesInRect:(CGRect)rect lineColor:(UIColor *)color
{
    ...
    // 设置绘图的属性
    CGContextSetLineWidth(ctx, self.CircleConnectLineWidth);
    // 线条颜色
    [color set];
    //渲染路径
