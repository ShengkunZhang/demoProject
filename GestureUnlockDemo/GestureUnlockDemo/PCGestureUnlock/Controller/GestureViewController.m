...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    // 进来先清空存的第一个密码
    [PCCircleViewConfig saveGesture:nil Key:gestureOneSaveKey];
}
...
- (void)setupSameUI
{
    ...
    msgLabel.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 14);
    msgLabel.center = CGPointMake([UIScreen mainScreen].bounds.size.width/2, CGRectGetMinY(lockView.frame) - 30);
...
- (void)setupSubViewsSettingVc
{
    ...
    infoView.center = CGPointMake([UIScreen mainScreen].bounds.size.width/2, CGRectGetMinY(self.msgLabel.frame) - CGRectGetHeight(infoView.frame)/2 - 10);
...
- (void)setupSubViewsLoginVc
{
    ...
    imageView.center = CGPointMake([UIScreen mainScreen].bounds.size.width/2, [UIScreen mainScreen].bounds.size.height/5);
    [imageView setImage:[UIImage imageNamed:@"head"]];
    [self.view addSubview:imageView];
    
    // 管理手势密码
    UIButton *leftBtn = [UIButton new];
    [self creatButton:leftBtn frame:CGRectMake(CircleViewEdgeMargin + 20, [UIScreen mainScreen].bounds.size.height - 60, [UIScreen mainScreen].bounds.size.width/2, 20) title:@"管理手势密码" alignment:UIControlContentHorizontalAlignmentLeft tag:buttonTagManager];
    
    // 登录其他账户
    UIButton *rightBtn = [UIButton new];
    [self creatButton:rightBtn frame:CGRectMake([UIScreen mainScreen].bounds.size.width/2 - CircleViewEdgeMargin - 20, [UIScreen mainScreen].bounds.size.height - 60, [UIScreen mainScreen].bounds.size.width/2, 20) title:@"登陆其他账户" alignment:UIControlContentHorizontalAlignmentRight tag:buttonTagForget];
}
...
{
    NSString *gestureOne = [PCCircleViewConfig getGestureWithKey:gestureOneSaveKey];
...
- (void)circleView:(PCCircleView *)view type:(CircleViewType)type didCompleteSetSecondGesture:(NSString *)gesture result:(BOOL)equal
{
    ...
    if (equal) {
        ...
        [PCCircleViewConfig saveGesture:gesture Key:gestureFinalSaveKey];
